...
namespace Time.Tracking.Jira
{
    ...
    internal class WatchTimer
    {
        ...
        /// <summary>
        /// Bumped every time <see cref="Reset"/> runs. Lets a caller that captured this before
        /// starting a long-running operation (e.g. posting a worklog) tell, once it comes back,
        /// whether the row was reset — and possibly restarted — in the meantime, so it knows
        /// not to stomp on a session it doesn't own.
        /// </summary>
        ...
        public void Start()
        {
            ...
            // A start with nothing on the clock begins a fresh session, so it gets a fresh
            // initial time — even if a stale one is still hanging around from a session that
            // was logged to Jira (which empties the clock) rather than Reset.
            if (initialStartTime == null || totalTime == TimeSpan.Zero)
            {
